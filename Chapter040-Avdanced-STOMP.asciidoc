...
As an example, we can use receipt to confirm that a consumer has been subscribed successfully to a destination. If the broker can not successfully create the subscription, it will send back an +ERROR+ frame to the client and close the connection. In practice, this means that a successful subscription is _silent_, the client is not informed of its success. We can use receipts to have an explicit confirmation of the subscription by adding a +receipt+ header when the client subscribes to a destination. The broker must then deliver a  +RECEIPT+ frame that will inform the client that the broker has processed its subscription successfully.

...
If we run the application, a STOMP message will be sent every time the location manager updates the device's location.
...
CONNECTED
heart-beat:0,0
session:ID:retsina.local-64018-1390843083020-2:7
...
These headers are added by the STOMP broker and provides additional metadata about the messages. We will explore some of them later in the <<ch_advanced_stomp>> and <<ch_beyond_stomp>> chapters.
====

...
However, being text-based means that the protocol is not the most efficient to use as it will takes less space on the network and in memory that corresponding binary-based protocols.
...
Messages will be sent every time the device location changes. This is a bit unconvenient when developing this application as I don't want to move around my home to change my location and send a message.

...
STOMP protocol is very simple with few frames to know to be able to use it.

The protocol does not define advanced features that are often provided by messaging brokers. However, it is possible to use them from STOMP
thanks to the support of non-standard additional headers.

In the example <<ex_stomp_receive2>>, the two messages received by the consumers were containing headers that have not been described: 
+timestamp+, +expires+ and +priority+.

The headers were added by ActiveMQ to provide additional information on the messages.
...
ActiveMQ defines other headers that can be add to the +SEND+ frame when sending a message (as descrided in (http://activemq.apache.org/stomp.html)[ActiveMQ STOMP page]).

[TODO]
====
Cross-reference with message persistence concept.
====
...
Most STOMP brokers (including ActiveMQ) defaults to _non-persistent_ messages. To send a persistent message to AcitveMQ broker,

...
I wrote this book to tell you about a simple and powerful technology that has been used with success in the enterprise for many years and starts to be used in other environments like the Web and mobile devices.
...
I don't intend to show that messaging protocol is *the* best transport protocol but instead that it's a _great_ complement that you can add to your tool box.
...
* a _consumer_ is an application that consume _messages_ *from* a _destination_
* a _broker_ is the server entity that will handle messages from producers and route them to the consumers.
...
They need to know about the broker to connect to it but they may even not connect to the same broker. A set of brokers can constitute a cluster and messages would flow from one to another before it is finally delivered to a consumer.
...
MQTT stands for MQ Telemetry Transport and really was created to broadcast data from small devices with low power usage and constrained memory resources.

Since these protocols are often simple, Messaging brokers often provide both of them. This allows to send a message using STOMP from an application and consume the same message from another application using MQTT. The applications are free to use the best messaging protocols for their needs and rely on the broker implementations for the interoperability.
Since these protocols are also interoperable, messages can also flow from one broker implementation to another without the need for any additional components such as bridges.
...
In this chapter, we introduce stomp.js, a JavaScript client for STOMP, and write a Web application that receives messages with the GPS from the iOS devices and display them on a map. This Web application will also send messages to the mobile app.

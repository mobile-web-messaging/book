...
* a _broker_ is the server entity that will handle messages from producers and distributed them to the consumers according to their destinations.

[[img_preface_1]]
.Messaging Concepts
image::images/messaging_concepts.png["Diagram of the messaging concepts"]
...
One key aspect of messaging is that it couples very loosely its participants. The sender and consumer know nothing of each other. When one application sends a message, it has no knowledge on when or where the message will be consumed and how many consumers will consume it. It also possible that the message will not be consumed at all if nobody has any interest for it.
